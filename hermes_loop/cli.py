...
def _list_examples():
    ...
    print()
    _comment("Structured JSON logs for programmatic consumption (pipe to jq, etc.)")
    _cmd('hermes_loop --goal "Fix tests" --json-logs --run')
    _cmd("hermes_loop --goal \"Batch process\" --json-logs --run 2>&1 | jq '.summary'")
    print()
