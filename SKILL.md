...
| `--no-tool-shortcut` | `false` | Inject anti-shortcut instructions into spawned worker prompts to force real tool execution. Prevents the model from fabricating tool results instead of actually calling tools. Useful for accurate tool-execution timing measurements. |
